...
| 🟢 可选改进 | 锦上添花/轻微问题：风格优化/次要重构/建议优化 | 步骤4/5 审查·精简发现的记入缺陷池 `docs/BUGS.md`（下个版本）；dev-bug 在范围内主动报告的留当前里程碑，由步骤3 处理 |

**核心约束**：🔴/🟡 一定进入当前里程碑、必须解决；🟢 为可选，按入口分流（审查发现→缺陷池，用户报且在范围内→当前版本）。
