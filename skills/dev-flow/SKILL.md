---
name: dev-flow
description: 里程碑开发流程编排。自动检测项目状态，串行驱动 8 步开发流程（写文档→设计核对→开发→精简→审查→测试→功能验收→提交）。
---
...
**Gitignore 处理**：若里程碑文档目录在 `.gitignore` 中（如 `.dev-flow/milestones/`），步骤1/8 跳过文档的 git 提交，只提交代码变更和版本文件。里程碑状态仅保存在本地文件中，不进入 git 历史。文档创建不受影响。
...
**人工审核**：仅在步骤2（设计核对）完成后可选暂停人工确认。步骤4/5/6/7 不设人工审核，由自动化审查直接判断门禁，不通过则自动打回开发阶段。

人工审核规则由 `AGENTS.md` 中 `## 设计审查配置` 段落的 `人工复核` 字段控制，三档取值：

| 取值 | 行为 |
|------|------|
| 关闭 | 步骤2 完成后不暂停，自动继续 |
| 开启 | 步骤2 完成后暂停，等待用户确认后再继续 |
| 可选（默认） | 步骤2 完成后，仅当报告结论为 ❌ 时暂停人工确认；全 ✅ 则自动继续 |
...
## 审查框架
步骤2和步骤7分别使用独立的审查/验收技能：

- **步骤2（开发前）**：调用 `devflow-review`（设计审查），检查里程碑文档 vs 产品文档，验证设计是否合理
- **步骤7（开发后）**：调用 `dev-acceptance`（功能验收），从 git diff 出发逐项验证代码实现是否真正满足需求，不信任流程标记
`AGENTS.md` 中定义的审查维度在步骤2设计审查中纳入检查；步骤7功能验收则从 git diff 出发逐项验证，不受审查维度约束。
...
**门禁**：报告结论为 ✅ 且人工审核已确认（如需）

**人工审核暂停**（仅步骤2）：根据 `AGENTS.md` 中 `## 设计审查配置` 的 `人工复核` 字段决定：
- `关闭`：不暂停，自动勾选步骤2
- `开启`：暂停并提示用户确认设计核对报告，用户确认后勾选步骤2
- `可选`（默认）：若报告结论为 ❌ 则暂停人工确认；全 ✅ 则自动勾选步骤2

> 步骤4/5/6/7 不设人工审核暂停，自动化审查直接判断门禁。
### 步骤3：开发（处理待办事项）
