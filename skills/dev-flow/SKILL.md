...
ELSE 找到当前里程碑（docs/milestones/ 中未完成文档按标识排序取最大：语义版本按 vX.Y.Z 数值比较，序号风格按 M 后数字比较）
  ...
  IF 步骤4 未勾选
    → 步骤4：代码精简（执行后检查报告，有 🔴/🟡 即打回开发阶段，仅有 🟢 入缺陷池不阻断）

  IF 步骤5 未勾选
    → 步骤5：代码审查（执行后检查报告，有 🔴/🟡 即打回开发阶段，仅有 🟢 入缺陷池不阻断）
...
状态列取值：⬜ = 待修复（dev-bug 写入或步骤4/5/6/7 打回时填入，状态机据此识别未修复 bug）；[x] = 已修复（步骤3 处理完后勾选）。优先级级别语义见「严重度分级契约」（步骤4/5 审查/精简发现的 🟢 记入缺陷池，不写入本表）。
```
...
| 🟢 可选改进 | 锦上添花/轻微问题：风格优化/次要重构/建议优化 | 步骤4/5 审查·精简发现的记入缺陷池 `docs/BUGS.md`；dev-bug 在范围内主动报告的留当前里程碑。缺陷池中的 🟢 在**下一个里程碑由 milestone-planner 规划时随 🔴/🟡 一并纳入**，所有遗留 bug 由该里程碑步骤3 按 🔴→🟡→🟢 处理完毕 |

**核心约束**：新里程碑规划时清空缺陷池，🔴/🟡/🟢 全部转入本里程碑并必须解决（顺序 🔴→🟡→🟢）；当前里程碑内审查/精简发现的 🟢 先入缺陷池、下个版本再处理，不阻断当前里程碑。
...
| 3 | 所有子任务标记为 ✅ 且 Bugs 表格中无未勾选行 | 继续开发（⬜ 含 🔴/🟡/🟢，均需处理；步骤4/5 的 🟢 已入缺陷池不在此表） |
| 4 | 精简无 🔴/🟡 发现（🟢 入缺陷池不阻断） | 🔴/🟡 写入 Bugs 表格，取消步骤3/4/5/6/7勾选，回到步骤3修复后重跑4→5→6→7 |
| 5 | 审查报告无 🔴/🟡 发现（🟢 入缺陷池不阻断） | 🔴/🟡 写入 Bugs 表格，取消步骤3/4/5/6/7勾选，回到步骤3修复后重跑4→5→6→7 |
...
1. 各步骤在门禁判断时已将该步发现登记进 Bugs 表格（🔴/🟡 入 Bugs、🟢 入缺陷池），本动作不再重复登记，仅执行以下状态回退。
...
4. 按优先级处理 bug：🔴（必须修复）→ 🟡（应该修复）→ 🟢（可选改进，仅限 dev-bug 在范围内主动报告的条目；步骤4/5 审查/精简发现的 🟢 已记入缺陷池，不在此表）
...
3. 将发现的每一处问题分级登记，便于追溯，并按级别决定走向：
   - 🔴/🟡：登记到里程碑文档的 Bugs 表格（来源=步骤4代码精简），状态 ⬜ 待修复：
     ...
     - 来源：步骤4代码精简
     - 描述：发现位置
   - 🟢：记录到缺陷池 `docs/BUGS.md`（格式见 dev-bug 技能「缺陷池格式」），来源标注「步骤4代码精简」，不写入当前里程碑、不打回。写入前按（来源+标题）去重：若缺陷池已有同来源同标题的 🟢 条目则跳过，避免重跑 4→5→6→7 时重复累积：
     - `| {当前里程碑版本} | 🟢 | {标题} | 步骤4代码精简 | {描述} |`

   **原则：🔴/🟡 发现问题即打回开发阶段解决，不在精简/审查步骤就地修改；🟢 可选改进记入缺陷池留给下个版本。**
4. 精简报告写入 `{version}-reports/step4-simplify.md`（记录每处发现与走向）
5. 门禁判断：
   - 存在 🔴/🟡 → 不勾选，按打回动作（步骤4/5/6/7）取消步骤3/4/5/6/7 勾选并回到步骤3 修复后重跑 4→5→6→7
   - 仅有 🟢（无 🔴/🟡）→ 🟢 已入缺陷池，不打回，勾选步骤4 继续
   - 无任何发现 → 勾选步骤4

**原则**：功能不变，只改组织方式；遵循项目现有风格；🔴/🟡 打回开发阶段，🟢 入缺陷池

**门禁**：精简无 🔴/🟡 发现（🟢 入缺陷池不阻断；🔴/🟡 任一即打回开发阶段）
...
2. 根据技能返回的结论决定后续：
   - **分级登记所有发现**：将报告中的问题列表 **🔴🟡🟢 全部** 处理，确保无一遗漏：
     - 🔴/🟡：登记到里程碑文档的 Bugs 表格（来源=步骤5代码审查），状态 ⬜ 待修复：
       ...
       - 来源：步骤5代码审查
       - 描述：详细问题描述
     - 🟢：记录到缺陷池 `docs/BUGS.md`（格式见 dev-bug 技能「缺陷池格式」），来源标注「步骤5代码审查」，不写入当前里程碑、不打回。写入前按（来源+标题）去重：若缺陷池已有同来源同标题的 🟢 条目则跳过，避免重跑 4→5→6→7 时重复累积：
       - `| {当前里程碑版本} | 🟢 | {标题} | 步骤5代码审查 | {描述} |`
   - **门禁判断**：
     - **存在 🔴/🟡**：打回开发阶段。已登记的 🔴/🟡 为 ⬜ 待修复 bug，按打回动作（步骤4/5/6/7）处理：取消步骤3/4/5/6/7 勾选并回到步骤3 修复（按 🔴→🟡 顺序），修复后依次重跑步骤4→5→6→7。🟢 已入缺陷池，不纳入本次回退。
     - **仅有 🟢（无 🔴/🟡）**：🟢 已入缺陷池，不打回、不阻断，勾选步骤5 继续。
     - **无任何发现**：勾选步骤5。

   > 注：🔴/🟡 打回开发阶段当前里程碑解决；🟢 是可选改进，记入缺陷池留给下个版本，不打断当前里程碑。

**门禁**：审查报告无 🔴/🟡 发现（🟢 入缺陷池不阻断；🔴/🟡 任一即打回开发阶段）
