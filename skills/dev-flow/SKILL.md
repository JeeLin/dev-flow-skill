...
**Bug 收集**：里程碑文档创建后，用户可随时通过以下方式提交 bug，Claude 写入当前里程碑文档的 Bugs 表格：

- **触发**：输入 `/bug <描述>`，或自然语言提及（如「有个bug：xxx」「记录一下 bug」）
- **Claude 处理**：
  1. 从描述中提取标题（一句话概括）
  2. 根据描述自动判断优先级：
     - 🔴 崩溃/无法使用/数据丢失/接口报错等严重问题
     - 🟡 偶发/功能异常/体验问题等中等问题
     - 🟢 建议优化/锦上添花等轻微问题
  3. 写入 Bugs 表格，来源标注为「用户反馈」
  4. 回复用户已记录，显示写入的内容
